...
function ShortestPathfindingVisualizer(props) {
  ...
  useEffect(() => {
    ...
    let path = []; // holds the shortest path (all nodes falling in it)
    ...
    const dfs = (sr, sc) => {
      ...
      if (
        sr - 1 != -1 &&
        btn[(sr - 1) * rows[0].getElementsByTagName('td').length + sc].value != '-1' &&
      ...
      if (
        ...
        btn[sr * rows[0].getElementsByTagName('td').length + (sc + 1)].value != '-1' &&
