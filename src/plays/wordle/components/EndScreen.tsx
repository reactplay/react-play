...
export default function EndScreen(props: { allTimeStats: AllTimeStats; reset: () => void }) {
  ...
  return (
    <div className="end-screen w-2/6 h-4/5 bg-default absolute z-10 rounded-lg flex flex-col items-center justify-center space-y-4 opacity-0 font-wordle">
        ...
        {statsTable.map((statistic) => {
          return (
            <div className="flex-1 h-full grid grid-rows-2">
              ...
              <p className="text-xs w-full flex items-center justify-center">{[statistic[0]]}</p>
            </div>
          );
        })}
      </div>
      <h4 className="text-left to-slate-200 font-bold text-lg w-3/5 pl-4">GUESS DISTRIBUTION</h4>
