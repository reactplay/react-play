...
function ImageCarousel(props: any) {
  ...
  const isSwipe = useCallback(
    () => Math.abs(endPos - startPos) >= threshold,
    [endPos, startPos]
  );
  ...
  const handleActiveSlide = useCallback(
    (direction: number = 0) => {
      ...
      if (direction === 0) {
        setActiveSlide((preV) =>
          activeSlide === data.length - 1 ? 0 : preV + 1
        );
      }
      if (direction === 1) {
        setActiveSlide((preV) =>
          activeSlide === 0 ? totalSlides - 1 : preV - 1
        );
      }
    },
